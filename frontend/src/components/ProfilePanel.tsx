      ...
      <aside className="ml-auto flex h-full w-full max-w-3xl flex-col overflow-hidden rounded-[2rem] border border-carbon-200 bg-gradient-to-br from-white/8 via-teal-500/5 to-cyan-500/4 text-carbon-900 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-6 sm:px-8">
          <div className="grid gap-6 lg:grid-cols-[1fr_0.9fr]">
            <section className="rounded-[1.75rem] border border-white/10 bg-gradient-to-br from-slate-950/45 via-cyan-950/20 to-teal-950/20 p-5 text-white">
              <h3 className="text-lg font-semibold text-white">General Info</h3>
              <p className="mt-1 text-sm text-white/75">This data is saved in your Supabase user metadata.</p>
              ...
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                <label className="sm:col-span-2">
                  ...
                  <div
                    ...
                    className={`rounded-2xl border-2 border-dashed px-4 py-6 text-sm text-center transition ${
                        dragActive ? "border-white/50 bg-white/10" : "border-white/20 bg-white/8"
                      }`}
                  >
                      <p className="text-white/85">Drop image here or choose a file</p>
                      <p className="mt-1 text-xs text-white/65">PNG, JPG, WEBP supported</p>
                    ...
                    <input
                      ...
                      className="mt-3 w-full rounded-2xl border border-white/20 bg-white/90 px-4 py-3 text-sm text-carbon-900 outline-none transition placeholder:text-carbon-500 focus:border-white/40"
                    />
                </label>
                ...
                <label>
                    <span className="mb-2 block text-sm font-medium text-white/90">Title / Role</span>
                  <select
                    value={title}
                  ...
                  >
                      <option value="" className="bg-white text-carbon-900">Select title</option>
                ...
                <label>
                    <span className="mb-2 block text-sm font-medium text-white/90">Organization</span>
                  <select
                    value={organization}
                  ...
                  >
                      <option value="" className="bg-white text-carbon-900">Select organization</option>
                ...
                <label className="sm:col-span-2">
                    <span className="mb-2 block text-sm font-medium text-white/90">Bio / Notes</span>
                    <textarea
                      value={bio}
                      onChange={(event) => setBio(event.target.value)}
                      rows={4}
                      placeholder="Anything useful for your profile"
                      className="w-full rounded-2xl border border-white/20 bg-white/90 px-4 py-3 text-sm text-carbon-900 outline-none transition placeholder:text-carbon-500 focus:border-white/40"
                    />
                </label>
              </div>
            </section>

            <section className="rounded-[1.75rem] border border-white/10 bg-gradient-to-br from-slate-950/45 via-teal-950/20 to-cyan-950/20 p-5 text-white">
              <h3 className="text-lg font-semibold text-white">Security</h3>
              <p className="mt-1 text-sm text-white/75">Change your email or password here.</p>
              ...
              <div className="mt-5 space-y-4">
                ...
                <label>
                  <span className="mb-2 block text-sm font-medium text-white/90">New password</span>
                  <input
                    type="password"
                    value={newPassword}
                ...
                <label>
                  <span className="mb-2 block text-sm font-medium text-white/90">Confirm new password</span>
                  <input
                    type="password"
                ...
                <div className="rounded-2xl border border-white/10 bg-black/25 px-4 py-3 text-xs leading-5 text-white/75">
          ...
          {message ? <p className="mt-5 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700">{message}</p> : null}

          <div className="mt-6 flex flex-wrap items-center justify-between gap-3 border-t border-white/10 pt-5">
            <p className="text-xs text-white/70">Profile data is stored in Supabase user metadata for this account.</p>
            <button
              type="submit"
              disabled={saving}
              className="rounded-2xl bg-accent-500 px-5 py-3 text-sm font-semibold text-carbon-900 transition hover:bg-accent-600 disabled:cursor-not-allowed disabled:opacity-60"
            >
